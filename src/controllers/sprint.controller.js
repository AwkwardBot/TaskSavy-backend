...
const createSprint = catchAsync(async (req, res) => {
    const sprint = await sprintService.createSprint(req.body, req.user._id, req.params.projectId);
...
const getSprints = catchAsync(async (req, res)=> {
    ...
    const sprints =  await sprintService.getSprints(req.user._id, req.params.projectId);
...
const getSprint = catchAsync(async (req, res) => {
    const sprint = await sprintService.getSprintById(req.params.sprintId, req.params.projectId, req.user._id);
    if (!sprint) {
